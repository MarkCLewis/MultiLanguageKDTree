...
/// every call.
pub fn quickstat_index_par<F: Fn(usize, usize) -> bool + Sync>(
    indices: &mut [usize],
    buffer: &mut [usize],
    goal: usize,
    lt: F,
    ...
    while s + 1 < e {
        ...
        let high_index = if chunk_size > MIN_CHUNK_SIZE {
            ...
            let cnts: Vec<(&[usize], usize)> = indices[s + 1..e]
                .par_chunks(chunk_size)
                .map(|inds| {
                    (
                        inds,
                        inds.into_iter()
                            .fold(0, |cnt, ind| cnt + if lt(*ind, indices[s]) { 1 } else { 0 }),
                    )
                })
                .collect();
            ...
            cnts_with_buffer
                .par_iter_mut()
                .for_each(|(inds, low_buffer, high_buffer)| {
                    let mut low = 0;
                    let mut high = 0;
                    for i in 0..inds.len() {
            ...
            indices[s..e].par_iter_mut()
                .zip(buffer[s..e].par_iter_mut())
                .for_each(|(i, b)| *i = *b);
            s + num_less
        } else {
            let mut low = s + 1;
...
// This version is horribly slow. Chunking makes it slower.
pub fn quickstat_index_mpsc<F: Fn(usize, usize) -> bool + Sync>(
    indices: &mut [usize],
    buffer: &mut [usize],
    goal: usize,
    lt: F,
    ...
    while s + 1 < e {
        ...
        let high_index = if chunk_size > MIN_CHUNK_SIZE {
            ...
            thread::scope( |scope| {
                scope.spawn( || {
                    indices[s + 1..e].par_iter().for_each_init(
                            || tx.clone(),
                        |tx, chunk| {
                            tx.send((*chunk, lt(*chunk, indices[s]))).unwrap();
                        },
                    )
                });
                while low < high {
                    let (ind, less) = rx.recv().unwrap();
                    if less {
                        buffer[low] = ind;
                        low += 1;
                    } else {
...
#[cfg(test)]
mod tests {
    use crate::quickstat::{quickstat_index, quickstat_index_par, quickstat_index_mpsc};
    ...
    fn random_slice_test() {
        ...
        for _ in 0..200 {
            ...
            quickstat_index(&mut indices[start..end], goal - start, |i1, i2| {
                vals[i1] < vals[i2]
            });
            for i in 0..n {
                if i < start {
    ...
    fn random_test_par() {
        ...
        for _ in 0..20 {
            let goal = fastrand::usize(0..n);
            quickstat_index_par(&mut indices, &mut buffer, goal, |i1, i2| {
                vals[i1] < vals[i2]
            });
            for i in 0..n {
                if i < goal {
    ...
    fn random_test_mpsc() {
        ...
        for _ in 0..20 {
            let goal = fastrand::usize(0..n);
            quickstat_index_mpsc(&mut indices, &mut buffer, goal, |i1, i2| {
                vals[i1] < vals[i2]
            });
            for i in 0..n {
                if i < goal {
