...
struct KDTree 
    # for leaves
    particles::Union{Nothing, Array{Int64}}
...
function build_tree(indices::Array{Int64}, start::Int64, ending::Int64, system::Array{Body})::KDTree
    ...
    if np <= MAX_PARTS
        node = KDTree(zeros(Int64, np), 0, 0.0, 0.0, [], 0.0, nothing, nothing)
        for i in 0:np-1
	...
	#println("Build internal")
        ...
        node = KDTree(nothing, split_dim, split_val, m, cm, size, left, right)
    ...
    function accel_recur(cur_node::KDTree, p::Int64, system::Vector{Body}, acc::Vector{Float64})
        if cur_node.particles != nothing
    ...
    function print_tree(step::Int64, tree::KDTree, system::Array{Body})
        function print_node(n::KDTree, file::IO)
            if cur_node.particles != nothing
